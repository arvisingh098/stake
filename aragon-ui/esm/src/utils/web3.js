...
var BSCScan_NETWORK_TYPES = new Map([['main', ''], ['kovan', 'kovan.'], ['rinkeby', 'rinkeby.'], ['ropsten', 'ropsten.'], ['goerli', 'goerli.']]);
var BSCScan_TYPES = new Map([['block', 'block'], ['transaction', 'tx'], ['address', 'address'], ['token', 'token']]);
var BLOCK_EXPLORERS = {
  BSCScan: function BSCScan(_ref) {
    var type = _ref.type,
        value = _ref.value,
    ...
    if (!BSCScan_NETWORK_TYPES.has(networkType)) {
      throw new Error('provider not supported.');
    }

    if (!BSCScan_TYPES.has(type)) {
      throw new Error('type not supported.');
    }

    var subdomain = BSCScan_NETWORK_TYPES.get(networkType);
    var typePart = BSCScan_TYPES.get(type);
    return "https://".concat(subdomain, "BSCScan.io/").concat(typePart, "/").concat(value);
  }
};
/**
 ...
 * @param {string} options.provider The explorer provider (e.g. BSCScan).
...
function blockExplorerUrl(type, value) {
  var _ref2 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
      ...
      provider = _ref2$provider === void 0 ? 'BSCScan' : _ref2$provider;
